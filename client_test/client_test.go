...
var _ = Describe("Client Tests", func() {
	...
	Describe("More integrity testing zzzz", func() {
		
		Specify("Integrity: Detect user data tampering", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Integrity: Detect file metadata tampering", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Integrity: Detect append structure tampering", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Integrity: Swap file contents between two files", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Integrity: Delete random datastore entries", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
	...
	Describe("Revocation edge cases", func() {
		...
		Specify("Revocation: Revoke before invitation accepted", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Revocation: Multiple revocations on same user", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Revocation: Revoked user cannot create invitations", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Revocation: Verify file re-encryption after revoke", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Revocation: Revoked user with saved UUIDs cannot read", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
	...
	Describe("Sharing edge cases", func() {
		...
		Specify("Sharing: Accept invitation for non-existent file", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Sharing: Create invitation for non-existent recipient", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Sharing: Create invitation for non-existent file", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Sharing: Accept invitation from wrong sender", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
			...
			// Bob tries to accept but claims it's from Charles
			err = bob.AcceptInvitation("charles", invite, bobFile)
			Expect(err).ToNot(BeNil(), "Should fail when sender is wrong")
		})
		
		Specify("Sharing: Multiple users share with same recipient", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
	...
	Describe("Error handling", func() {
		...
		Specify("Error: Load file that doesn't exist", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Error: Append to file that doesn't exist", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Error: Accept same invitation twice", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Edge case: Empty filename", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
			
			// Store file with empty filename
			err = alice.StoreFile("", []byte(contentOne))
			Expect(err).To(BeNil())
			
			// Load file with empty filename
			data, err := alice.LoadFile("")
			Expect(err).To(BeNil())
			Expect(data).To(Equal([]byte(contentOne)))
		})
		
		Specify("Edge case: Very long filename", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Edge case: Empty file content", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Edge case: Overwrite file multiple times", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Concurrent sessions: Multiple sessions modify same file", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Password: Wrong password should fail", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Password: Empty password", func() {
			alice, err = client.InitUser("alice", "")
			Expect(err).To(BeNil())
		...
		Specify("Username: Case sensitivity", func() {
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
			
			// Different case should be different user
			Alice, err := client.InitUser("Alice", defaultPassword)
			Expect(err).To(BeNil())
	...
	Describe("File bandwidth diagnostics", func() {
		...
		Specify("DIAGNOSTIC: Bandwidth should scale with content size", func() {
			...
			// bw2 should be roughly 90 bytes more than bw1 (100-10)
			// bw3 should be roughly 900 bytes more than bw2 (1000-100)
			diff1 := bw2 - bw1
			diff2 := bw3 - bw2
			
			Expect(diff1).To(BeNumerically(">", 50))  // At least 50 bytes difference
			Expect(diff2).To(BeNumerically(">", 500)) // At least 500 bytes difference
			Expect(diff2).To(BeNumerically(">", diff1)) // diff2 should be larger than diff1
		})
		...
		Specify("DIAGNOSTIC: File size should not affect append bandwidth", func() {
			// Test 1: Small file (fresh user)
			alice, err = client.InitUser("alice1", defaultPassword)
			Expect(err).To(BeNil())
			
			err = alice.StoreFile("test.txt", []byte("tiny"))
			Expect(err).To(BeNil())
			
			bwSmall := userlib.DatastoreGetBandwidth()
			err = alice.AppendToFile("test.txt", []byte("X"))
			Expect(err).To(BeNil())
			bwSmall = userlib.DatastoreGetBandwidth() - bwSmall
			
			// Test 2: Large file (different fresh user)
			bob, err := client.InitUser("bob1", defaultPassword)
			Expect(err).To(BeNil())
			
			largeContent := make([]byte, 100000)
			err = bob.StoreFile("test.txt", largeContent)
			Expect(err).To(BeNil())
			
			bwLarge := userlib.DatastoreGetBandwidth()
			err = bob.AppendToFile("test.txt", []byte("X"))
			Expect(err).To(BeNil())
			bwLarge = userlib.DatastoreGetBandwidth() - bwLarge
			
			fmt.Printf("Small: %d, Large: %d, Diff: %d\n", bwSmall, bwLarge, bwLarge-bwSmall)
