...
func decryptInvitation(hybridInv *HybridInvStruct, recipientPrivKey userlib.PKEDecKey) (*InvStruct, error) {
	// First decrypt without verification to get the SharerUsername
	// We need to do a partial decrypt to get the username
	symKey, err := userlib.PKEDec(recipientPrivKey, hybridInv.RSAEncryptedSymKey)
	if err != nil {
		return nil, err
	}
	...
	// Now get the sharer's verify key and verify the signature
...
func (userdata *User) createNewFile(filename string, content []byte, userStruct *UserStruct) error {
	...
	// Create FileChunkContent
	chunkUUID := uuid.New()

	// Encrypt content
	encryptedContent, err := encryptThenMAC(content, fileEncKey, fileMACKey)
	if err != nil {
		return err
	}
...
func (userdata *User) overwriteFile(filename string, content []byte, userStruct *UserStruct) error {
	...
	// Create new FileChunkContent with new content
	chunkUUID := uuid.New()
	...
	fileContent.HeadUUID = chunkUUID
	fileContent.TailUUID = chunkUUID
...
func (userdata *User) AppendToFile(filename string, content []byte) error {
	...
	// Create new chunk for appended content
	...
	// Update the old tail chunk to point to new chunk
	...
	oldTailChunk.NextUUID = &newChunkUUID

	// Store updated old tail
	...
	// Update FileContent tail pointer
	fileContent.TailUUID = newChunkUUID
